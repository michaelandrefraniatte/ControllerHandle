...
namespace JoyconChargingGripsAPI
{
    public class JoyconChargingGrip
    {
        ...
        private FileStream fStreamLeft, fStreamRight;
        ...
        private void taskDLeft()
        {
            for (; ; )
            {
                ...
                try
                {
                    fStreamLeft.Read(report_bufLeft, 0, report_bufLeft.Length);
        ...
        private void taskDRight()
        {
            for (; ; )
            {
                ...
                try
                {
                    fStreamRight.Read(report_bufRight, 0, report_bufRight.Length);
        ...
        private bool AttachGripLeftController(string path)
        {
            try
            {
                handleLeft = CreateFile(path, FileAccess.ReadWrite, FileShare.ReadWrite, IntPtr.Zero, FileMode.Open, (uint)EFileAttributes.Overlapped, IntPtr.Zero);
                ...
                fStreamLeft = new FileStream(handleLeft, FileAccess.Read, report_bufLeft.Length, true);
                return true;
            }
            catch { return false; }
        }
        private void SubcommandGripLeftController(byte sc, byte[] buf)
        {
            ...
            buf_[1] = buf[1];
            buf_[0] = buf[0];
            HidD_SetOutputReport(handleLeft.DangerousGetHandle(), buf_, 12);
        }
        private bool AttachGripRightController(string path)
        {
            try
            {
                handleRight = CreateFile(path, FileAccess.ReadWrite, FileShare.ReadWrite, IntPtr.Zero, FileMode.Open, (uint)EFileAttributes.Overlapped, IntPtr.Zero);
                ...
                fStreamRight = new FileStream(handleRight, FileAccess.Read, report_bufRight.Length, true);
                return true;
            }
            catch { return false; }
        }
        private void SubcommandGripRightController(byte sc, byte[] buf)
        {
            ...
            buf_[1] = buf[1];
            buf_[0] = buf[0];
            HidD_SetOutputReport(handleRight.DangerousGetHandle(), buf_, 12);
        }
    }
}